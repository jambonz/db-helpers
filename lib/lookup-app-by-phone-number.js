...
async function lookupAppByPhoneNumber(pool, logger, phoneNumber, voip_carrier_sid) {
  ...
  if (r.length > 0) {
    const obj = r[0];
    debug(`querying phone_numbers object is : ${JSON.stringify(obj)}`);
  ...
  if (voip_carrier_sid) {
    const r2 = await pool.query('SELECT application_sid from voip_carriers where voip_carrier_sid = ?', {
      raw: true,
